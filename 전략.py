    ...
    try:
        ...
        for i in range(4,399):
            ...
            if(buyflag):
                ...
                if(df.iloc[i-1]["lowPrice"]>df.iloc[i]["tradePrice"]):
                    ...
                    if(price<=100):
                        ...
                        price = price-100
                    print("구매가격 :",100000/sonik,", 판매가격 :",price,", 이익 :",sonik * price - 100000)
                   
                    if(sonik * price - 100000 >= 0):
                        plus = plus + 1
                        if(topplus<sonik *price - 100000):
                            topplus = sonik *price - 100000
                    else:
                        minus = minus + 1
                        if(topminus>sonik *price - 100000):
                            topminus = sonik *price - 100000
                    sonik =  sonik * price - 100000
                    money = money + sonik
                    buyflag = True
                    cnt = cnt + 1
                if(df.iloc[i-1]["lowPrice"]>df.iloc[i]["tradePrice"]):
